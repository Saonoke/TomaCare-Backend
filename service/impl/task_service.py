...
class TaskService(TaskServiceMeta):
    ...
    def insert_task(self,tasks:list[TaskShow],plant_id:int):
        tanggal = date.today()
        dates = [tanggal + timedelta(days=i) for i in range(8)]
        task = []
        for tanggal,tugas in zip(dates,tasks):
            tugas = TaskCreate(**tugas.dict(),plant_id=plant_id,tanggal=tanggal)
            task.append(self._task_repository.create(tugas))
        return task

    def create_task(self, penyakit:penyakitEnum , plant_id:int) -> list[TaskShow]:
    # id plant
        try:
